...
5. **費用 › ＋対象を追加** — 毎月かかるもの（家賃・給与・通信・リース…）を登録し、月額を入れる
   - **支払先**を選びます（一覧に無ければフォームの中から新しい取引先を作れます）
   - **払い方**を決めます … 即払い／**買掛**＝毎月の支払請求を作り、期日と残高で管理
   → 「定期をすべて→12」で 今月から年度末までの空いているマスを一気に埋められます
6. **予実管理 › 入力 › 実績** — 会計事務所の **試算表** を見ながら、費用の実績（税抜）を月ごとに打つ
   （売上の実績は請求書から自動なので、灰色のまま触りません）
7. あとは **入金・支払** を記録するだけ。資金繰り・OKRの数字は自動で埋まります。
...
|---|---|
| ダッシュボード | — |
| 回収（売掛金） | 売掛金 ／ 入金 ／ 入金チェック ／ 督促 |
| 支払（買掛金） | 買掛金 ／ 支払請求 ／ 支払実行 ／ 資金繰り |
| 取引先 | 一覧（すべて） ／ 支払先 |
| 費用 | 費用表 ／ 物件（建物） |
...
### 費用管理は「表1枚」（2026-09-16 作り直し）
**費用 › 費用表** は、これから出ていくお金の **予定表** です（実績ではありません）。

```
売上原価                       ← 大分類（畳める）
├ 外注費                       ← 中分類
│  └ 支援業務の外注  支払先 ／ 物件 ／ 月額 ／ 8月…7月 ／ 年間計 ／ →12
販売費及び一般管理費
└ 地代家賃 › 社宅・寮 家賃 › さくら寮A棟 家賃 …
```

- 行は `大分類 › 中分類 › 小分類 › 対象`。科目の行はその下の合計で、▼ で畳めます（開閉は記憶します）
- マスは **税込の予定額**。打つと保存され、空・0 にすると消えます。開始月・終了月の外は `—`
- **→12** … その月から年度末まで、**空いているマスだけ**を「左にある直近の数字（無ければ月額）」で埋めます。
  入っている数字は変えません。ツールの「定期をすべて→12」で全部の定期にまとめて実行できます
- 買掛の対象は、この予定額で **支払請求**（月まとめ）を作れます（マスが空なら月額）
- **費用表の数字は予実の実績に入りません**。実績は 予実 › 入力 › 実績（試算表から手入力）

### 物件（建物）— 借りている建物ごとのお金
社宅・寮・事務所・駐車場を1件ずつ登録し、**貸主・契約期間・家賃・共益費・駐車場代・敷金**を持たせます。
費用表の対象に **物件** を選ぶと、家賃・電気・水道・ガスがその建物に集まり、
**物件 × 12か月の予定**の表になります（行を押すと対象ごとの内訳）。
契約満了の **90日前** から画面と 🔔 で知らせます。更新・解約の連絡には時間が要るためです。

### デモデータ（設定 › 管理者のみ）
画面の動きを確かめるための作り物を、ボタン1つで入れられます。
取引先25社・物件4件・前年度12か月ぶんの請求/入金/支払請求/支払/費用表の予定/実績など約900件。
...
| 売上の実績は | **保存しません**。請求書から毎回計算します |
| 費用の実績は | 会計事務所の**試算表**を見て、予実 › 入力 › 実績に**手で入れます**（税抜・2026-09-16〜） |
| 費用表は | **予定（見込み）**であって実績ではありません。支払請求・旧「経費」も実績には入りません |
...
## Money Forward 連携（請求・入金・試算表を「読むだけ」）

請求書は **MF クラウド請求書**、仕訳は **経理（会計事務所）が MF クラウド会計** で入れます。
この システムは 売掛金の運用（請求の確認・入金・消込・督促・突合）だけを持ち、**MF には一切書きません**。

- 毎日 06:00（日本時間）に自動で「請求 → 入金 → 自動消込」。時刻・ON/OFF は 設定 › API・AI連携 ›「設定を変える」（管理者）。
  手で動かすときは同じ画面の「今すぐ同期」。1種類だけ取り込むときは「請求を取り込む」「入金を取り込む」（中身を見てから実行できます）。
- API が無くても **CSV** で同じことができます（請求／入出金明細／試算表）。
- 経理へは 回収 › 入金 › ツール ›「消込一覧CSV」を渡します。
- 月末は 回収 › 突合 で 試算表の売掛金と突き合わせ、合ったら **締める**（締めた月は自動同期でも動きません）。

手順（アプリ登録・.env・CSV の列・困ったとき）は **docs/MONEYFORWARD.md**。
